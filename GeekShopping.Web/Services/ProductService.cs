...
namespace GeekShopping.Web.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<IEnumerable<ProductViewModel>> FindAllProducts(string token)
        {
            ...
            var parsedResponse = await response.ReadContentAs<List<ProductViewModel>>();
            return parsedResponse ?? new List<ProductViewModel>();
        }
        ...
        public async Task<ProductViewModel> CreateProduct(ProductViewModel model, string token)
        {
            ...
            if (response.IsSuccessStatusCode)
                return await response.ReadContentAs<ProductViewModel>() ?? throw new ApplicationException("Unexpected error");
            else
